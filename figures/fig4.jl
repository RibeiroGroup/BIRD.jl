function fig4()
    ...
    hideydecorations!(hf_ax2, grid=false, ticks=false, ticklabels=false)

    # Inset axis
    hf_inax = Axis(gd[1,1], width=Relative(0.65), height=Relative(0.65), halign=0.8, valign=0.8, backgroundcolor=:grey90,
    xticks=_latexthis([3400, 3500, 3600]), yticks=_latexthis([0.8, 1.0, 1.2, 1.4, 1.6]),
    xticklabelsize=15, yticklabelsize=15)

    hidedecorations!(hf_inax, label=false, ticklabels=false, ticks=false)
    ...
    # Inset axis
    lih_inax = Axis(gd[2,1], width=Relative(0.65), height=Relative(0.65), halign=0.8, valign=0.8, backgroundcolor=:grey90,
    xticks=_latexthis([1350, 1400, 1450]), yticks=_latexthis([0.8, 1.0, 1.2, 1.4, 1.6]),
    xticklabelsize=15, yticklabelsize=15)
    hidedecorations!(lih_inax, label=false, ticklabels=false, ticks=false)
...
function sc_hf!(ax1, ax2, inax, ci;T=4000)
    ...
    text!(inax, overt+2, 0.72, text=L"%$ci \rightarrow 23", align=(:left, :top), fontsize=15, rotation=π/2)
...
function sc_lih!(ax1, ax2, inax, ci;T=2000)
    ...
    vlines!(inax, [overt], linestyle=:dot, color=:firebrick)
    text!(inax, overt+2, 0.72, text=L"%$ci \rightarrow 29", align=(:left, :top), fontsize=15, rotation=π/2)

    axislegend(ax2, L"$\Omega_R$ (cm$^{-1}$)", merge=true, position=:rb, labelsize=20)
end

function kploss_fig4()
    ...
    kploss_sc_hf!(hf_ax1, hf_ax2, hf_ci)
...
function kploss_sc_hf!(ax1, ax2, ci;T=4000)
    ...
    rvals, e, C = BIRD.get_continuum_states(hf)
    kpl0 = BIRD.compute_kploss(hf_dipole, BIRD.D0, hf.nmax, rvals, e, C, 4000)

    # Get free space rate
    J0 = get_transport_matrix(E, V, DOS0, T, kploss=kpl0)
    k0 = eigmin(J0)
    println("k0: ", k0)
    ...
    kplc = BIRD.compute_kploss(hf_dipole, x->BIRD.Dc(x, L*μm2Å), hf.nmax, rvals, e, C, 4000)
    Jc = get_transport_matrix(E, V, DOSc, T, kploss=kplc)
    kc = eigmin(Jc)
    ...
    for (i,νM) in enumerate(νMvals)
        ...
        kplp = BIRD.compute_kploss(hf_dipole, x->BIRD.Dp(x, L*μm2Å, ν2ω(νM), ν2ω(400)), hf.nmax, rvals, e, C, 4000)
        DOSp = get_DOS_matrix(E, L=L*μm2Å, g=400, νM=νM, regime=2)
        if any(isinf, DOSp)
            println(νM)
        end
        Jp = get_transport_matrix(E, V, DOSp, T, kploss=kplp)
        kp = eigmin(Jp)
